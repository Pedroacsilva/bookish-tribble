...
fig5 = figure(5);
plot(-1*MAXLAG:MAXLAG, r_hat); grid on; xlabel('k'); ylabel('r_{hat}[k]'); title('Autocorrelação do processo x'); xlim([-1*MAXLAG MAXLAG]);%Como meto um chapeu no r no title?
%Observamos neste plot que a autocorrelação do processo é um impulso de
%Dirac
...
MSG = ['Estimamos a potencia deste processo: ', num2str(mean(x.^2)), '.'];
disp(MSG)
...
%x = x(1:200);
fig6 = figure(6);
plot(x); title('harmonicproc.wav'); xlabel('n'); ylabel('x[n]'); xlim([0 200]);
...
MAXLAG = 70;
x_corr = xcorr(x, MAXLAG, 'biased');
fig8 = figure(8);
plot(-1*MAXLAG:MAXLAG, x_corr); grid on; xlabel('k'); ylabel('r_{hat}[k]'); title('Autocorrelação do sinal harmonicproc.wav'); xlim([-1*MAXLAG MAXLAG]);
